...
(in contrast to [Other File Types](#other-file-types) which require a data conversion step, either explicitly or implicitly when using the Vitessce R or Python packages).
[Plugin file types](/docs/dev-plugins/#plugin-file-types) can be defined to facilitate loading custom file types or those that Vitessce does not yet support.
...
`}
  ...
  genes are needed for the heatmap.  Similarly, selecting a gene to display over a scatterplot or spatial view can be costly
